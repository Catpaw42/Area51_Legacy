...
class MaxHeap2 {
	...
	public void increaseKey(int i, int k) {
		if (k<0){
			System.err.println("key negative");
		} else {
			elements[i]+=k;
		}
...
public class MaxHeapDriver2
{
	public static void main(String[] args) throws IOException {
		new MaxHeapDriver2().run();
	}
	...
	private void run() throws IOException {
		...
		for (int i = 0; i < N; i++) {
			...
			if (type.equals("IK")) {
				maxHeap.increaseKey(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()));
